    ...
    def laserscan_callback(self, msg):
        ...
        if(self.laser_count < 200):
            ...
            if(self.laser_count == 200):
                ...
                print("thread2-excecute---delta_len:", self.delta_len)
    ...
    def euler_angle_callback(self, msg):
        ...
        if(self.count_euler < 500):
            ...
            if(self.count_euler == 500):
                ...
                self.count_euler += 1
                self.init_angel += self.turn_angel
...
def turn():
    ...
    x_flag = 1
    z_flag = 1
    distance_corrector.length_move = finale_length

    #停下转弯
    while(1):
        x_flag = 0
        z_flag = 1
        if(abs(distance_corrector.delta_len) < 0.01):
            distance_corrector.clear()
            direction_corrector.clear()
            direction_corrector.turn_angel = 3.1415927/2
            distance_corrector.length_move = 0
            break
    ...
    while(1):
        x_flag = 1
        z_flag = 1
        if(abs(direction_corrector.delta_ang) < 0.03):
            distance_corrector.clear()
            direction_corrector.clear()
            direction_corrector.turn_angel = 0
            distance_corrector.length_move = dist_cross
            break
    ...
    while(1):
        if(abs(direction_corrector.delta_ang) < 0.03):
            del direction_corrector
            del distance_corrector
            break
...
if __name__ == '__main__':
    ...
    thread1 = Thread(target=turn)
    thread2 = Thread(target=ros_spin)
    thread1.start()
    thread2.start()
