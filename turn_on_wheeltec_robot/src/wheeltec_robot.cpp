...
void turn_on_robot::update_number(){
    int last_pos=0, plus=0, is_last_pos_in_right=0;
    ...
    if(Change_Area!=0)
    {
      ...
      else if (Change_Area==2)
      {
        Pot_Pos = 11;
        Pot_Counter = 11;
      }
      else if (Change_Area==3)
      {
        Pot_Pos = 23;
        Pot_Counter = 23;
      }
      else if (Change_Area==4)
      {
        Pot_Pos = 35;
        Pot_Counter = 35;
      }
    ...
    if(Pot_Pos==63) //initial
    {
        Pot_Pos = 0;//start_point
    }
    else if (Pot_Pos < 23) //0-23
        Pot_Pos++;
    else //24-41
    {
        /*
        ...
        R -> nextL: +1, R -> nextR: +2
        */
        if(Pot_Pos <= 35) //record last pos
        {
          last_pos = Pot_Pos;
          is_last_pos_in_right = last_pos % 2;
        }
        else
        {
          last_pos = 36 + int((Pot_Pos-36)/3);
          is_last_pos_in_right = last_pos % 2;
        }


        //find plus value
        if(Pot_Counter%2 == 0) //下一个花盆在同排
        {
          if(is_last_pos_in_right)
            plus = -1;
          else
            plus = 1;
        }
        ...
        {
          plus = Arrive_Info; //1-L, 2-R
          if(is_last_pos_in_right == 0) //in L, ++
            plus++;
        }

        //update Pot_Pos
        if(Pot_Pos < 35) //24-35, 35 is in R
          Pot_Pos += plus;
        else{//36-41
          Pot_Pos = 35 + (last_pos-35)*3 + (plus - 1) * 3 + 1;
          color_fire = 1;
          if(Arrive_Info==1)
            Color_Info = 61;
          else if(Arrive_Info==2)
            Color_Info = 62;
        }
    }
    ...
    if(Pot_Counter==100)
        ...
        Pot_Counter++;//计数，下个花盆在——偶数：同排；奇数：下一排
}

uint8_t turn_on_robot::update_distance(){
    float distance;
    if(Pot_Pos<12 && Pot_Pos%2==1)
        US_Data = 60 - 17.5 - US_Data;//right = total - wide - left
    else if(Pot_Pos<24 && Pot_Pos%2==1)
        US_Data = 70 - 17.5 - US_Data;//right = total - wide - left

    //distance: 实际值-理想值(cm)
    if(Pot_Pos<12){
        distance = US_Data - 26.0;
    }
    else if(Pot_Pos<24){
        distance = US_Data - 34.0;
    }
    else if(Pot_Pos<36){
        distance = US_Data - 37.0;
    }
    else{
        distance = US_Data - 30.0;
    }

    if(distance>=0){
        dist_mm = int(distance*10);
        dist_sign = 0;
    }
    else{
        dist_mm = int(-distance*10);
...
uint8_t turn_on_robot::Get_Pot_Info()
{
    //Arrive_Info: 0-23(N-0 Y-1), 24-42(N-0 L-1 R-2) -> all
    //if Distance>C, N, else Y
    const int N = 20;
    uint8_t info = 0;

    if(isPumpWorking == 0)
    {
        if(Arrive_Info != 0){ //start signal
            if(pump_fire==0)
            {
                update_number();
                update_distance();
        ...
        else if(Silent_Counter>=N) //end signal
        {
            pump_fire = 0;
            Arrive_State = 0;
            if(Pot_Pos<24 && Pot_Pos%2==0)
                Arrive_Info = 1;

            std_msgs::Int32 W;
            W.data = 1;
            if(Pot_Pos<24 && Pot_Pos%2==1) //after water right
                water_publisher.publish(W);
            else if(Pot_Pos>=24)
                water_publisher.publish(W);
        }

        if(Silent_Counter < N+2)
          Silent_Counter++;
    }
    ...
    if(is_arm_OK==1)
    {
      arm_publisher.publish(W);
      is_arm_OK = 0;
    }

    //now in D && arrived && updated
    if(36<=Pot_Pos && Pot_Pos<60 && color_fire==1 && Color_Info<3){
        Pot_Pos += Color_Info;
        color_fire = 0;
    }
  
    info = (Arrive_State<<7) + (Pot_Pos<<1) + dist_sign;
    if(color_fire==1){
      info = (Arrive_State<<7) + (Color_Info<<1) + dist_sign;
    }

    std::cout<<"-------Write Info-----------"<<endl;
    std::cout<<"color_info:"<<int(Color_Info)<<endl;
    std::cout<<"Pump:"<<int(isPumpWorking)<<endl;
    std::cout<<"Arrive_Info:"<<int(Arrive_Info)<<endl;
    std::cout<<"Arrive_State:"<<int(Arrive_State)<<endl;
    std::cout<<"Pot_Pos:"<<int(Pot_Pos)<<endl;
    std::cout<<"Silent_Counter:"<<int(Silent_Counter)<<endl;
    std::cout<<"US_Data:"<<US_Data<<endl;
    std::cout<<"dist_sign:"<<int(dist_sign)<<endl;
    std::cout<<"dist_mm:"<<int(dist_mm)<<endl;
    return info;
}


void turn_on_robot::Arrived_Callback(const std_msgs::Int32 &Arrived_Info_msg){
  Arrive_Info = Arrived_Info_msg.data;
}
void turn_on_robot::US_Data_Callback(const std_msgs::Float32 &US_Data_Info_msg){
  US_Data = US_Data_Info_msg.data;
}
void turn_on_robot::Color_Callback(const std_msgs::Int32 &Color_msg){
  Color_Info = Color_msg.data;
}
void turn_on_robot::Area_Callback(const std_msgs::Int32 &Area_msg){
  Change_Area = Area_msg.data;
}
/**************************************
Date: January 28, 2021
...
void turn_on_robot::Cmd_Vel_Callback(const geometry_msgs::Twist &twist_aux)
{
  ...
  uint8_t Pot_Info = Get_Pot_Info();
  Send_Data.tx[9] = Pot_Info;
  Send_Data.tx[10] = dist_mm;
  std::cout<<"Pot_Info:"<<int(Pot_Info)<<endl;

  Send_Data.tx[11] = Check_Sum(9, SEND_DATA_CHECK); // For the BCC check bits, see the Check_Sum function //BCC校验位，规则参见Check_Sum函数
  Send_Data.tx[12] = FRAME_TAIL;                   // frame tail 0x7D //帧尾0X7D
  try
  {
...
bool turn_on_robot::Get_Sensor_Data_New()
{
  ...
  //View the received raw data directly and debug it for use//直接查看接收到的原始数据，调试使用
  /*ROS_INFO_STREAM("%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x",
  Receive_Data_Pr[0],Receive_Data_Pr[1],Receive_Data_Pr[2],Receive_Data_Pr[3],Receive_Data_Pr[4],Receive_Data_Pr[5],Receive_Data_Pr[6],Receive_Data_Pr[7],
  Receive_Data_Pr[8],Receive_Data_Pr[9],Receive_Data_Pr[10],Receive_Data_Pr[11],Receive_Data_Pr[12],Receive_Data_Pr[13],Receive_Data_Pr[14],Receive_Data_Pr[15],
  Receive_Data_Pr[16],Receive_Data_Pr[17],Receive_Data_Pr[18],Receive_Data_Pr[19],Receive_Data_Pr[20],Receive_Data_Pr[21],Receive_Data_Pr[22],Receive_Data_Pr[23]);
  */
  ...
  Receive_Data.Frame_Header = Receive_Data.rx[0]; // The first part of the data is the frame header 0X7B //数据的第一位是帧头0X7B
  Receive_Data.Frame_Tail = Receive_Data.rx[24];  // The la st bit of data is frame tail 0X7D //数据的最后一位是帧尾0X7D

  if (Receive_Data_Pr[0] == FRAME_HEADER || count > 0) // Ensure that the first data in the array is FRAME_HEADER //确保数组第一个数据为FRAME_HEADER
    count++;
  else
    count = 0;
  if (count == 25) // Verify the length of the packet //验证数据包的长度
  {
    ...
    {
      check = Check_Sum(23, READ_DATA_CHECK); // BCC check passes or two packets are interlaced //BCC校验通过或者两组数据包交错

      if (check == Receive_Data.rx[23])
      {
      ...
      if (error == 0)
      {
        ...
        */
        ...
        //水泵工作状态
        STM_state = Receive_Data.rx[22];
        isPumpWorking = STM_state%2;
        is_arm_OK = STM_state>>2;
        return true;
      }
    }
  }
...
turn_on_robot::turn_on_robot() : Sampling_Time(0), Power_voltage(0)
{
  ...
  water_publisher = n.advertise<std_msgs::Int32>("water", 10);
  arm_publisher = n.advertise<std_msgs::Int32>("arm_arrive", 10);
  ...
  Arrived_Sub = n.subscribe("arrive_info", 10, &turn_on_robot::Arrived_Callback, this);
  US_Data_Sub = n.subscribe("us_data", 10, &turn_on_robot::US_Data_Callback, this);
  Color_Sub = n.subscribe("color", 10, &turn_on_robot::Color_Callback, this);
  Area_Sub = n.subscribe("area", 10, &turn_on_robot::Area_Callback, this);
  ...
  {
    ROS_INFO_STREAM("start debugging");
...
turn_on_robot::~turn_on_robot()
{
  ...
  Send_Data.tx[9] = 0;//Pot_Info   1 bits  6 bits
  Send_Data.tx[10] = 0;//US_Data   


  Send_Data.tx[11] = Check_Sum(9, SEND_DATA_CHECK); // Check the bits for the Check_Sum function //校验位，规则参见Check_Sum函数
  Send_Data.tx[12] = FRAME_TAIL;
  try
  {
