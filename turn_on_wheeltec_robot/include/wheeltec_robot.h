...
#define SEND_DATA_SIZE    13         //The length of data sent by ROS to the lower machine //ROS向下位机发送的数据的长度
...
class turn_on_robot
{
	...
	private:
		...
		void Arrived_Callback(const std_msgs::Int32 &Arrived_Info_msg);
		void US_Data_Callback(const std_msgs::Float32 &US_Data_Info_msg);
		void Color_Callback(const std_msgs::Int32 &Color_msg);
		void Area_Callback(const std_msgs::Int32 &Area_msg);         

		ros::Publisher odom_publisher, imu_publisher, voltage_publisher, water_publisher; //Initialize the topic publisher //初始化话题发布者
		...
		uint8_t Get_Pot_Info();
		uint8_t Get_US_Data();
		void update_number();
		uint8_t update_distance();
		...
		float Odom_Trans(uint8_t Data_High,uint8_t Data_Low); //Odometer data is converted to read //里程计数据转化读取
	        ...
	        //通讯消息
	        uint8_t Arrive_Info=0, Color_Info=0;
	        uint8_t dist_sign=0, isPumpWorking=0, dist_mm=0;
	        float US_Data;
	        int is_arm_OK, STM_state, Change_Area=0;

	        //中间状态变量
	        uint8_t Pot_Pos, Pot_Counter, pump_fire=0, color_fire=0;
	        uint8_t Arrive_State=0, Silent_Counter;
};
#endif
